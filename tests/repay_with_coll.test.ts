...
describe('Repay with collateral SDK tests', function () {
    it('repay_with_coll_partial_non_sol', async function () {
      ...
      // diff comes from IR we add when repaying
      assertFuzzyEq(
        lamportsToNumberDecimal(new Fraction(obligationBefore.state.borrows[0].borrowedAmountSf).toDecimal(), kaminoMarket.getReserveByMint(debtTokenMint)?.state.liquidity.mintDecimals.toNumber()!).sub(amountToRepay),
        lamportsToNumberDecimal(new Fraction(obligationAfter.state.borrows[0].borrowedAmountSf).toDecimal(), kaminoMarket.getReserveByMint(debtTokenMint)?.state.liquidity.mintDecimals.toNumber()!),
        0.001
      );

      assertFuzzyEq(
        lamportsToNumberDecimal(obligationBefore.state.deposits[0].depositedAmount.toNumber(), kaminoMarket.getReserveByMint(collTokenMint)?.state.liquidity.mintDecimals.toNumber()!).sub(amountToRepay.mul(1 + slippagePct / 100).mul(await getPriceMock(kaminoMarket, debtTokenMint, collTokenMint))),
        lamportsToNumberDecimal(obligationAfter.state.deposits[0].depositedAmount.toNumber(), kaminoMarket.getReserveByMint(collTokenMint)?.state.liquidity.mintDecimals.toNumber()!),
        0.001
      );
    });
