...
describe('Main lending market instruction tests', function () {
  ...
  it('borrow_max_sol', async function () {
...

    {
      ...
      const maxBorrow = obligation!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
...

    {
      ...
      const maxBorrowSol = obligation!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
      solReserve = kaminoMarket.getReserveBySymbol('SOL');
      const borrowFactor = solReserve!.getBorrowFactor();
      ...
      const maxBorrowUsdh = obligation!.getMaxBorrowAmount(kaminoMarket, usdhMint);
...

    {
      ...
      await kaminoMarket.reloadSingleReserve(solReserve!.address);
      const maxBorrowBefore = obligation!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
      await sleep(5000);
      await kaminoMarket.reloadSingleReserve(solReserve!.address);
      ...
      const maxBorrowAfter = obligationAfterRefresh!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
...

    {
      ...
      obligation = await kaminoMarket.getObligationByWallet(depositor.publicKey, new VanillaObligation(PROGRAM_ID));
      const maxBorrowAfter = obligation!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
      assert.ok(maxBorrowAfter > maxBorrowBefore);
...

    {
      ...
      const maxBorrow = newBorrowerObligation!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
      ...
      const maxBorrowAfterDebt = obligation!.getMaxBorrowAmount(kaminoMarket, NATIVE_MINT);
  ...
  it('withdraw_max_sol', async function () {
    ...
    const {
      ...
      firstMint: usdhMint,
      secondMint: _,
    } = await createMarketWithTwoReserves(depositSymbol, borrowSymbol, false);
    ...
    const depositor = await newUser(env, kaminoMarket, [
      [depositSymbol, depositUsdhAmount],
      [borrowSymbol, new Decimal(1)],
    ]);
...

    {
      ...
      const maxWithdraw = obligation!.getMaxWithdrawAmount(kaminoMarket, usdhMint);
    ...
    const newDepositor = await newUser(env, kaminoMarket, [
      [borrowSymbol, borrowSolAmount.add(new Decimal(1))],
...

    {
      ...
      const maxWithdraw = obligation!.getMaxWithdrawAmount(kaminoMarket, NATIVE_MINT);
      assert.equal(maxWithdraw.toNumber(), 0);

      obligation = await kaminoMarket.getObligationByWallet(depositor.publicKey, new VanillaObligation(PROGRAM_ID));

      const maxUsdhWithdraw = obligation!.getMaxWithdrawAmount(kaminoMarket, usdhMint);

      assert.ok(maxUsdhWithdraw.lt(depositUsdhAmount.mul(depositReserve.getMintFactor())));
    }

    {
      // Ltv 0 for USDH, still can't withdraw
      const buffer = Buffer.alloc(32);
