...
export class Pool {
  ...
  static async fetch(
    ...
    if (info.programAddress !== programId) {
      throw new Error(
        `PoolFields account ${address} belongs to wrong program ${info.programAddress}, expected ${programId}`
      )
    }
  ...
  static async fetchMultiple(
    ...
    return infos.map((info) => {
      ...
      if (info.programAddress !== programId) {
        throw new Error(
          `PoolFields account ${info.address} belongs to wrong program ${info.programAddress}, expected ${programId}`
        )
      }
